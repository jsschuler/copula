 ...
 * (h-function inverse), derived from C(v|u) = u^{-theta-1}
 * (u^{-theta}+v^{-theta}-1)^{-1/theta-1} -- the same construction and
 * derivation as tests/test_parametric_tail.c's clayton_hinv, duplicated
 * here per this test suite's established self-containment convention
 * (see the PRNG note above). */
...
static void make_clayton_data(size_t n, double theta, uint64_t seed, double *u0, double *u1) {
    rng_seed(seed);
    for (size_t i = 0; i < n; i++) {
        double u = rng_uniform();
        double p = rng_uniform();
        double v = clayton_hinv(u, p, theta);
        u0[i] = fmin(fmax(u, 1e-6), 1.0 - 1e-6);
        u1[i] = fmin(fmax(v, 1e-6), 1.0 - 1e-6);
    }
}

/* Exact Gumbel-copula samples via bisection on the closed-form
 * conditional CDF -- same construction as
 * tests/test_parametric_tail.c's gumbel_hinv, duplicated here likewise. */
static double gumbel_h(double u, double v, double theta) {
    double x = -log(u);
    double y = -log(v);
    double log_A = log(pow(x, theta) + pow(y, theta)) / theta;
    double A = exp(log_A);
    return exp(-A - log(u) + (1.0 - theta) * log_A + (theta - 1.0) * log(x));
}
...
static double gumbel_hinv(double u, double p, double theta) {
    ...
    for (int iter = 0; iter < 100; iter++) {
        ...
        if (gumbel_h(u, mid, theta) < p) lo = mid; else hi = mid;
    }
    return 0.5 * (lo + hi);
}

static void make_gumbel_data(size_t n, double theta, uint64_t seed, double *u0, double *u1) {
    rng_seed(seed);
    for (size_t i = 0; i < n; i++) {
        double u = rng_uniform();
        double p = rng_uniform();
        double v = gumbel_hinv(u, p, theta);
        u0[i] = fmin(fmax(u, 1e-6), 1.0 - 1e-6);
        u1[i] = fmin(fmax(v, 1e-6), 1.0 - 1e-6);
    }
}
...
static void test_evt_splice_default_matches_unweighted(void) {
    size_t n = 500, d = 2;
    double *u0 = (double *)malloc(n * sizeof(double));
    double *u1 = (double *)malloc(n * sizeof(double));
    make_clayton_data(n, 3.0, 81, u0, u1);
    double *u = (double *)malloc(n * d * sizeof(double));
    uint8_t *mask = (uint8_t *)malloc(n * d);
    memcpy(&u[0 * n], u0, n * sizeof(double));
    memcpy(&u[1 * n], u1, n * sizeof(double));
    ...
    hdcd_local_fit_options_t opt = default_options(9);
    HDCD_CHECK_NEAR(opt.evt_splice_gate, 0.0, 1e-15); /* memset default, not an explicit default_options() line */
    size_t parent = 0;
    hdcd_local_fit_t *fit = NULL;
    HDCD_CHECK(hdcd_local_fit_node(u, mask, n, d, 1, &parent, 1, &opt, &fit) == HDCD_OK);
    HDCD_CHECK(hdcd_local_fit_tail_family(fit, 0) == HDCD_TAIL_FAMILY_NONE);
    HDCD_CHECK(isnan(hdcd_local_fit_tail_theta(fit, 0)));

    hdcd_local_fit_free(fit);
    free(u0); free(u1); free(u); free(mask);
    HDCD_PASS("evt_splice_gate=0 (the default) leaves every edge unspliced, exactly as before this option existed");
}

static void test_evt_splice_selects_clayton_on_clayton_data(void) {
    size_t n = 2000, d = 2;
    double true_theta = 3.0;
    double *u0 = (double *)malloc(n * sizeof(double));
    double *u1 = (double *)malloc(n * sizeof(double));
    make_clayton_data(n, true_theta, 82, u0, u1);
    double *u = (double *)malloc(n * d * sizeof(double));
    uint8_t *mask = (uint8_t *)malloc(n * d);
    memcpy(&u[0 * n], u0, n * sizeof(double));
    memcpy(&u[1 * n], u1, n * sizeof(double));
    ...
    hdcd_local_fit_options_t opt = default_options(10);
    opt.evt_splice_gate = 0.1;
    size_t parent = 0;
    hdcd_local_fit_t *fit = NULL;
    HDCD_CHECK(hdcd_local_fit_node(u, mask, n, d, 1, &parent, 1, &opt, &fit) == HDCD_OK);
    HDCD_CHECK(hdcd_local_fit_tail_family(fit, 0) == HDCD_TAIL_FAMILY_CLAYTON);
    double fitted_theta = hdcd_local_fit_tail_theta(fit, 0);
    HDCD_CHECK(!isnan(fitted_theta));
    HDCD_CHECK(fabs(fitted_theta - true_theta) < 0.5); /* MLE'd on this node's own train split, not the full n */
    HDCD_CHECK(hdcd_local_fit_sinkhorn_converged(fit)); /* the blended kernel still normalizes to a valid copula */

    hdcd_local_fit_free(fit);
    free(u0); free(u1); free(u); free(mask);
    HDCD_PASS("evt_splice selects Clayton and recovers a plausible theta on genuinely Clayton-tail-dependent data");
}

static void test_evt_splice_selects_gumbel_on_gumbel_data(void) {
    size_t n = 2000, d = 2;
    double true_theta = 2.5;
    double *u0 = (double *)malloc(n * sizeof(double));
    double *u1 = (double *)malloc(n * sizeof(double));
    make_gumbel_data(n, true_theta, 83, u0, u1);
    double *u = (double *)malloc(n * d * sizeof(double));
    uint8_t *mask = (uint8_t *)malloc(n * d);
    memcpy(&u[0 * n], u0, n * sizeof(double));
    memcpy(&u[1 * n], u1, n * sizeof(double));
    ...
    hdcd_local_fit_t *fit = NULL;
    HDCD_CHECK(hdcd_local_fit_node(u, mask, n, d, 1, &parent, 1, &opt, &fit) == HDCD_OK);
    HDCD_CHECK(hdcd_local_fit_tail_family(fit, 0) == HDCD_TAIL_FAMILY_GUMBEL);
    double fitted_theta = hdcd_local_fit_tail_theta(fit, 0);
    HDCD_CHECK(!isnan(fitted_theta));
    HDCD_CHECK(fabs(fitted_theta - true_theta) < 0.5);
    HDCD_CHECK(hdcd_local_fit_sinkhorn_converged(fit));

    hdcd_local_fit_free(fit);
    free(u0); free(u1); free(u); free(mask);
    HDCD_PASS("evt_splice selects Gumbel and recovers a plausible theta on genuinely Gumbel-tail-dependent data");
}

static void test_evt_splice_gated_off_matches_unspliced(void) {
    /* A high gate that ordinary Gaussian data (no tail dependence at
     * all) never clears: falls back to NO splice for that edge, and the
     * fit must exactly match a plain (evt_splice_gate=0) fit. */
    size_t n = 500, d = 2;
    double *u0 = (double *)malloc(n * sizeof(double));
    double *u1 = (double *)malloc(n * sizeof(double));
    make_gaussian_copula_data(n, 0.6, 84, u0, u1);
    double *u = (double *)malloc(n * d * sizeof(double));
    uint8_t *mask = (uint8_t *)malloc(n * d);
    memcpy(&u[0 * n], u0, n * sizeof(double));
    memcpy(&u[1 * n], u1, n * sizeof(double));
    ...
    hdcd_local_fit_options_t flat_opt = default_options(12);
    size_t parent = 0;
    hdcd_local_fit_t *flat_fit = NULL;
    HDCD_CHECK(hdcd_local_fit_node(u, mask, n, d, 1, &parent, 1, &flat_opt, &flat_fit) == HDCD_OK);

    hdcd_local_fit_options_t gated_opt = flat_opt;
    gated_opt.evt_splice_gate = 0.9;
    hdcd_local_fit_t *gated_fit = NULL;
    HDCD_CHECK(hdcd_local_fit_node(u, mask, n, d, 1, &parent, 1, &gated_opt, &gated_fit) == HDCD_OK);

    HDCD_CHECK(hdcd_local_fit_tail_family(gated_fit, 0) == HDCD_TAIL_FAMILY_NONE);
    HDCD_CHECK_NEAR(hdcd_local_fit_holdout_score(gated_fit), hdcd_local_fit_holdout_score(flat_fit), 1e-9);

    hdcd_local_fit_free(flat_fit);
    hdcd_local_fit_free(gated_fit);
    free(u0); free(u1); free(u); free(mask);
    HDCD_PASS("evt_splice_gate is skipped (result matches the unspliced fit) when no edge clears it");
}

static void test_evt_splice_root_node_trivial(void) {
    size_t n = 20, d = 2;
    double u[40];
    uint8_t mask[40];
    for (size_t i = 0; i < 40; i++) { u[i] = 0.5; mask[i] = 1; }
    ...
    hdcd_local_fit_free(fit);
    HDCD_PASS("root node's tail-splice diagnostics are trivial (nothing to select)");
}

static void test_evt_splice_invalid_arguments(void) {
    size_t n = 500, d = 2;
    double *u0 = (double *)malloc(n * sizeof(double));
    double *u1 = (double *)malloc(n * sizeof(double));
    make_gaussian_copula_data(n, 0.5, 85, u0, u1);
    double *u = (double *)malloc(n * d * sizeof(double));
    uint8_t *mask = (uint8_t *)malloc(n * d);
    memcpy(&u[0 * n], u0, n * sizeof(double));
    memcpy(&u[1 * n], u1, n * sizeof(double));
    ...
    bad_negative_gate.evt_splice_gate = -0.1;
    ...
    bad_high_gate.evt_splice_gate = 1.5;
    ...
    bad_bandwidth.evt_splice_gate = 0.1;
    bad_bandwidth.evt_splice_bandwidth = -0.05;
    HDCD_CHECK(hdcd_local_fit_node(u, mask, n, d, 1, &parent, 1, &bad_bandwidth, &fit) == HDCD_ERROR_INVALID_ARGUMENT);

    free(u0); free(u1); free(u); free(mask);
    HDCD_PASS("evt_splice_gate outside [0,1] and a negative evt_splice_bandwidth are rejected");
}
...
int main(void) {
    ...
    test_evt_splice_default_matches_unweighted();
    test_evt_splice_selects_clayton_on_clayton_data();
    test_evt_splice_selects_gumbel_on_gumbel_data();
    test_evt_splice_gated_off_matches_unspliced();
    test_evt_splice_root_node_trivial();
    test_evt_splice_invalid_arguments();
